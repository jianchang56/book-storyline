...
2. 对 TXT、Markdown、分章目录或 EPUB，优先运行预处理脚本：
   ...
   单文件未识别出章节标题时，脚本默认停止；仅在确认原书确实为单章时追加
   `--allow-single-chapter`，不要把识别失败静默当成单章。
...
`manifest.json` 的 `chapter_detection` 会记录章节来自正文标题、文件边界、EPUB spine，还是显式单章确认。发布时必须把 manifest 交给校验器核对，不能只相信人工填写的章节数。
